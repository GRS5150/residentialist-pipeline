...
// ============================================================
// BAD POOL A SOURCES TO REMOVE (per product)
// These are sources misassigned to Pool A that aren't actually
// about the product's quality.
// ============================================================
const BAD_SOURCES = {
  "Marvin Signature Ultimate": [
    "fine-homebuilding-summit-2025",        // FHB Summit event page
    "what-is-building-science",             // BSC generic article
    "budget-friendly-windows-that-you-can"  // About Andersen 100, not Marvin
  ],
  "Reliabilt 3500": [
    "fine-homebuilding-summit-2025",        // FHB Summit event page
    "whats-definition-green-building"       // Generic green building article
  ],
  "Window World 4000": [
    "window-weatherstripping",              // Generic weatherstripping article
    "what-would-you-consider-to-be-a-pretty-good-window", // "window world" = industry, not brand
    "the-bs-beer-show-the-wondrous-world-of-windows",     // General windows podcast
    "greenbuildingadvisor-com",             // FHB subscription question
    "a-buyers-guide-to-windows",            // Generic buyer's guide
    "european-windows-that-open-outwards",  // European windows article
    "do-europeans-make-better-windows-than-we-do"  // European vs American comparison
  ],
  "Andersen 400 Series": [
    "pella-impervia-casement-windows"       // About Pella U-factor, not Andersen
  ],
  "Pella Impervia": []  // All look legitimate
};

function isBadSource(src, productName) {
  const badFragments = BAD_SOURCES[productName] || [];
  if (badFragments.length === 0) return false;
  ...
  for (const frag of badFragments) {
    ...
    if (summary.includes(frag.toLowerCase())) return true;
  }
  return false;
}

// V3 classifier (binary recommend/caution)
...
${numbered}`
    ...
    process.stdout.write(`  V3 ${Math.min(i + batchSize, sources.length)}/${sources.length}\r`);
  }
  console.log();
  return results;
}
...
${numbered}`
    ...
    process.stdout.write(`  V4 ${Math.min(i + batchSize, sources.length)}/${sources.length}\r`);
  }
  console.log();
  return results;
}

// Scoring with optional caution cap for Pool S/A minor negatives
function scorePC(sources, getSentimentValue, useCautionCap = false) {
  ...
  for (const poolKey of ["S", "A", "B", "C"]) {
    ...
    for (const src of poolSources) {
      ...
      // CAUTION CAP: For expert-tier sources (Pool S or A) with minor negative
      // sentiment (-0.5 = the "-1" on 5-point scale), reduce their weight to 0.75x
      // instead of full pool weight. Prevents "only downside vs X" from carrying
      // disproportionate negative influence at expert tier.
      if (useCautionCap && (poolKey === "S" || poolKey === "A")) {
        if (sentimentValue < 0 && sentimentValue > -1.0) {
          // Minor caution — reduce weight to 0.75
          finalWeight = 0.75 * credWeight;
        }
      }
...
async function main() {
  ...
  console.log("===============================================================");
  console.log("  V4 CLEAN TEST: Cleaned data + 5-point scale + caution cap");
  console.log("  - Bad Pool A sources removed across ALL products");
  console.log("  - 5-point nuanced scale with expert tone awareness");
  console.log("  - Minor caution from experts capped at 0.75x weight");
  console.log("===============================================================\n");
  ...
  for (const prod of products) {
    ...
    // Clean bad Pool A sources
    const badFrags = BAD_SOURCES[prod.name] || [];
    let removedCount = 0;
    ...
    const poolACleaned = sources.filter(s => (s.pool || "C").toUpperCase() === "A").length;
    console.log(`  Original: ${allSources.length} | Cleaned: ${sources.length} (removed ${removedCount} bad Pool A)`);
    console.log(`  Pool A remaining: ${poolACleaned} | Scoreable: ${scoreable.length}`);

    // Run V3 baseline on cleaned data
    console.log("\n  --- V3 (binary rec) on cleaned data ---");
    ...
    // Run V4 on cleaned data
    console.log("  --- V4 (5-point + cap) on cleaned data ---");
    ...
    // V4 without caution cap (for comparison)
    ...
    console.log(`\n  V4(cap): Score ${v4Score.score} | V4(raw): ${v4NoCap.score} | Quality: ${v4Actionable} | Pos: ${v4Pos} | Neg: ${v4Neg}`);
    ...
    console.log(`\n  DELTA: V3 ${v3Score.score} -> V4cap ${v4Score.score} (${(v4Score.score - v3Score.score) >= 0 ? "+" : ""}${(v4Score.score - v3Score.score).toFixed(2)})`);
  }

  // Summary table
  console.log("\n\n===============================================================");
  console.log("  COMPARISON TABLE (all on cleaned data)");
  console.log("===============================================================\n");
  console.log(`  ${"Product".padEnd(28)} ${"Rmvd".padStart(4)} ${"PoolA".padStart(5)} ${"V3".padStart(6)} ${"V4cap".padStart(6)} ${"V4raw".padStart(6)} ${"Δ".padStart(6)} ${"CapΔ".padStart(5)}`);
  console.log(`  ${"-".repeat(28)} ${"-".repeat(4)} ${"-".repeat(5)} ${"-".repeat(6)} ${"-".repeat(6)} ${"-".repeat(6)} ${"-".repeat(6)} ${"-".repeat(5)}`);

  const sorted = [...allResults].sort((a, b) => b.v4Score.score - a.v4Score.score);
  for (const r of sorted) {
    const delta = r.v4Score.score - r.v3Score.score;
    const capDelta = r.v4Score.score - r.v4NoCap.score;
    console.log(`  ${r.name.padEnd(28)} ${String(r.removed).padStart(4)} ${String(r.poolARemaining).padStart(5)} ${r.v3Score.score.toFixed(2).padStart(6)} ${r.v4Score.score.toFixed(2).padStart(6)} ${r.v4NoCap.score.toFixed(2).padStart(6)} ${((delta >= 0 ? "+" : "") + delta.toFixed(2)).padStart(6)} ${((capDelta >= 0 ? "+" : "") + capDelta.toFixed(2)).padStart(5)}`);
  }
  ...
  console.log(`\n  Score spread: V3 ${v3Range.toFixed(2)} -> V4cap ${v4Range.toFixed(2)} (${v4Range > v3Range ? "+" : ""}${(v4Range - v3Range).toFixed(2)})`);
  ...
  fs.writeFileSync(
    path.join(__dirname, "outputs", "staging_v4_clean_results.json"),
    JSON.stringify(allResults, null, 2)
  );
  console.log("  Results saved to outputs/staging_v4_clean_results.json");
}
