# Baseline Research: Window World 4000 (vinyl-dh)
# Auto-generated: 2026-03-16T17:00:50.564Z
# This file is automatically loaded on re-runs to avoid redundant web research.
# To force a full re-research, delete this file.
# Sections marked [LOCKED] are preserved on refresh runs.

I'll conduct a refresh research on the Window World 4000 DH configuration, verifying the baseline claims and searching for new information. Let me begin with key verification searches and new data discovery.
Now let me search for recent complaints, reviews, and field sources:
Based on my comprehensive refresh research, I can now provide an updated complete findings document for the Window World 4000 DH configuration. The baseline research data appears accurate, and I've found some additional recent information and confirmations.

# WINDOW WORLD 4000 DH - RESEARCH FINDINGS (UPDATED)

## 1. PRODUCT OVERVIEW


Window World 4000 series is manufactured by MI Windows (Metal Industries), with the 4000 series being the MI 1600, described as "a rather flimsy window."
 The product is configured as a double-hung (DH) window in vinyl construction. 
The 4000 series has manufacturing variability, with some locations previously using Alside windows, though Window World has transitioned to MI Windows manufacturing.
 Country of origin: United States (franchise model with third-party manufacturing partners).

## 2. CONFIRMED FINDINGS

### Performance Specifications (NFRC-certified values):
- **U-factor**: 
0.34 (4000 double hung with a low E)

- **SHGC**: 
0.25 (4000 double hung with a low E)

- **Visible Transmittance (VT)**: 
0.40 (4000 double hung with a low E)

- **Air Infiltration**: NOT PUBLISHED by manufacturer in accessible specifications

### Frame and Construction:
- **Frame Material**: Virgin vinyl frames
- **Spacer System**: PPG Intercept™ Warm Edge Spacer System
- **Glazing System**: Dual-pane, double-strength insulated glass
- **Weatherstripping**: Quad 4 weatherstripping at the header
- **Meeting Rail**: Metal reinforcement for added strength at the meeting rail

### Performance Classification:
- **Energy Star Certification**: 
All Window World double hung windows are ENERGY STAR® Certified

- **Good Housekeeping Seal**: Window World windows hold the Good Housekeeping Seal with two-year limited warranty

### Warranty Terms:
- **Product Warranty**: 
Limited lifetime warranty that's comparable to Champion's but shorter than Andersen's 20-year coverage

- **Labor Coverage**: Five-year labor guarantee on all vinyl window series 3000 or above
- **Transferability**: Transferable warranty with $250 transfer fee
- **Labor Transfer**: Labor warranty does not transfer to new homeowner

### Pricing:
- **Current Market Price**: 
Window World customers nationwide reported paying an average of $373 per window, or about $5,766 per project

- **4000 Series Range**: 
Consumers can expect the 4000 series to range in price from $325 to $575 fully installed for a normal sized opening


## 3. UNKNOWN / NOT DISCLOSED

- **AAMA Performance Class and Grade** — NOT PUBLISHED by Window World. 
While AAMA certification requires testing for air infiltration resistance, water resistance, and structural performance per AAMA/WDMA/CSA 101/I.S.2/A440 standards
, Window World does not publish specific AAMA certification details for the 4000 series.
- **Specific air infiltration rate** — NOT PUBLISHED in accessible technical specifications
- **IGU seal system details (dual vs single)** — NOT PUBLISHED
- **Glazing bead construction** — NOT PUBLISHED
- **Exact weatherstripping attachment method** — NOT PUBLISHED beyond general descriptions

## 4. RED FINDINGS

### Manufacturing Quality Concerns:

Professional assessment indicates the 4000 series "is actually the MI 1600. Its a rather flimsy window."
 **INSTALL-CONFOUND: LOW** - This represents a design/material quality concern rather than installation.

### Product Performance Assessment:

Independent reviewer assessment: "Generally not a vinyl series I would recommend for homeowners looking to stay in their homes long term. If consumers are considering WW, I would recommend upgrading to the company's higher end model."


### No Active Recalls Found:
Search of CPSC database found no active recalls for Window World products as of March 2026. 
Recent window recalls in 2026 have been limited to specific Andersen 100 Series casement window control devices
, not affecting Window World products.

## 5. YELLOW FINDINGS

### Manufacturing Variability:

Consumer confusion reported: "On the WW website, cross reference is made to an AMI window and a MIWD xact window relating to the 4000 series based on select features"
 - indicates potential regional manufacturing variations.


Some locations previously offered models equivalent to "Alside Mezzo" with "very low SHGC and relatively low VT rating. Unless you're in the deep south you may want to look for an option that will let in more light."


### Mixed Service Reviews:

Recent service complaint: "Window World cancelled on me and told me my home was too old for them to even look at. You would think that true professionals would look, evaluate, and assess, but nope, just a phone call saying your home is too old."
 (July 2025)

Positive service experience: 
Customer report: "I had a Window World door-sized window get broken by a lawnmower rock. Window World replaced [it] at no charge on a very timely basis, and installers were on time and professionals."
 (September 2025)

## 6. CONFIDENCE ASSESSMENT

**MODERATE** — Core performance specifications are consistently reported across multiple sources and align with industry standards for entry-level vinyl windows. The NFRC ratings appear reliable, though specific AAMA structural certifications remain unpublished. Manufacturing transition from Alside to MI Windows creates some uncertainty about current production quality, and professional assessments suggest this is a basic-grade product suitable for budget-conscious consumers but not recommended for long-term homeowners seeking premium performance.

## 7. FIELD SOURCE OPINIONS

### Professional Window Forum Assessments:

Window professional assessment: "Neither are great or considered top performers or even above average. You can do better."
 (Regarding Window World 4000 vs Harvey Tribute comparison)


Professional forum consensus: "Window world and Alside mezzo are lower end windows. You wont find any pro's here that will endorse either one."


**Credibility Assessment**: These sources appear to be verified window professionals on specialized forums with consistent technical vocabulary and multi-brand knowledge. **Field consensus diverges from marketing materials**, confirming entry-level positioning.

## 8. INTERNATIONAL CERTIFICATIONS

No international certifications found beyond U.S. ENERGY STAR qualification. Product appears to be manufactured and sold exclusively for the U.S. market through the Window World franchise network.

## 9. SPECIALTY FORUM FINDINGS

No qualified specialty forum sources (GBA, r/PassiveHouse, r/buildingscience) identified for this specific product during the research period. The Window World 4000 series appears to target the mainstream replacement market rather than high-performance building communities.

## 10. INSTALLATION CONFOUND FLAGS

### Service and Installation Quality Variability:

Industry context: "Unfortunately, these complaints are relatively common among many window companies, not just Window World. Many top window installers outsource work to contractors, so your specific experience may vary depending on your location."
 **INSTALL-CONFOUND: HIGH** - Franchise model creates installation quality variability.

### Product vs Service Issues:
Material quality concerns flagged as **INSTALL-CONFOUND: LOW** as these relate to manufacturing rather than installation practices. Professional assessments of the product as "flimsy" represent inherent design limitations rather than installation-dependent failures.

---

**Updated March 16, 2026** - This refresh confirms baseline data accuracy while adding recent pricing information, manufacturing clarifications, and current professional assessments. No new safety concerns or recalls identified.